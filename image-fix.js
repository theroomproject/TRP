...
const TRP_MASTER_VISUAL='https://raw.githubusercontent.com/theroomproject/TRP/main/assets/images/trp-originals-master.jpg?v=20260823i';
...
function openTRPTextRoom(name){
  ...
  if(wrap){
    wrap.querySelectorAll('.trp-room-canvas,.modal-image-error').forEach(el=>el.remove());
  ...
  if(img){
    ...
    img.onload=()=>{if(wrap)wrap.querySelectorAll('.modal-image-error').forEach(el=>el.remove());};
    img.onerror=()=>{if(wrap&&!wrap.querySelector('.modal-image-error')){const e=document.createElement('div');e.className='modal-image-error';e.textContent='Visual temporarily unavailable';wrap.appendChild(e);}};
